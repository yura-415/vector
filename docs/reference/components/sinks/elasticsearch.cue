...
components: sinks: elasticsearch: {
	title:       "Elasticsearch"
	description: """
		[Elasticsearch](\(urls.elasticsearch)) is a search engine based on the Lucene library. It
		provides a distributed, multitenant-capable full-text search engine with an HTTP web
		interface and schema-free JSON documents. Due to this feature set Elasticsearch is very
		commonly used to store and analyze log data. It ships with [Kibana](\(urls.kibana)), a
		simple interface for visualizing and exploring data in Elasticsearch.
		"""
